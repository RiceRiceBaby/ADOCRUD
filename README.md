...
###<b>Limitations:</b><br />
\#1. Does not support nested transactions, but does support nested connections. In other words, you can create a using context within another using context, but if both contexts call their commmit method and the outer context fails and the inner commit succeeds, the changse in that inner commit will still be applied to the database while the outer commit will roll back its changes.<br />
